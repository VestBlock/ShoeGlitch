          ...
          <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(360px,1fr)] xl:items-start">
            <article className="rounded-[2rem] border border-ink/10 bg-white/86 p-6 shadow-[0_24px_80px_rgba(10,15,31,0.07)] backdrop-blur-xl md:p-8">
              ...
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href={`/customer/watchlist?sku=${encodeURIComponent(model.item.sku)}&brand=${encodeURIComponent(model.item.brand)}&model=${encodeURIComponent(model.item.silhouette)}&colorway=${encodeURIComponent(model.item.colorway)}`} className="btn-glitch">
                  Join release alerts
                </Link>
                <Link href={`/releases/${model.item.slug}`} className="btn-outline">
                  Open release brief
                </Link>
                <Link href="/customer/watchlist" className="btn-outline">
                  Open watchlist
                </Link>
              </div>
            </article>
